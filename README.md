...
- 選考ステージを複数登録: カジュアル面談・書類選考・一次面接・二次面接・最終面接・内定/不合格/辞退など、それぞれに日程・実施方法・面接官・会話内容・印象・結果を記録
- 「現在のステータス」は選考ステージから自動で算出(日程が一番新しいステージが表示される)。別立ての管理は不要で、選考が進むたびにステージを追加するだけ
- トップページで次の選考予定を一覧表示
...
- `interview_stages`: 会社に紐づく選考ステージ(ステージ名、選考日程、実施方法、面接官名、会話内容、面接の印象、結果、Googleカレンダー予定ID)。「現在のステータス」はここから自動算出され、別のテーブルは持ちません
