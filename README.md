...
### 7. Googleカレンダー連携(追加設定不要)

選考ステータスの実施日時・各選考ステージの日程には「Googleカレンダーに追加」リンクが表示されます。これはGoogle Calendarの「かんたん追加」URL方式で、APIキーやOAuth登録は一切不要です。クリックするとGoogleカレンダーの予定作成画面が開く(内容は入力済み)ので、内容を確認して保存するだけです。

## データモデル

- `companies`: 会社名、会社情報、企業サイト、応募経路、ステータス(カジュアル面談/書類選考/一次面接/二次面接/三次面接/選考中/内定/不合格/辞退)とその実施日時、年収、勤務地、最寄駅、リモート可能日数(週1〜5日)、福利厚生、残業時間の目安、求人要件、志望順位・志望理由、決め手・懸念点
