...
「設定」ページでGoogleカレンダーと連携すると、**ステータス履歴を追加したときや選考ステージの日程を登録したときに、自動でGoogleカレンダーに予定が作成・更新・削除されます**(手動でリンクを押す必要はありません)。連携していない場合は、代わりに「Googleカレンダーに追加」リンク(クリックで予定作成画面が開く、登録不要の簡易版)が表示されます。
...
- `companies`: 会社名、会社情報、企業サイト、応募経路、現在の選考ステータス(カジュアル面談/書類選考/一次面接/二次面接/三次面接/選考中/内定/不合格/辞退)、年収、勤務地、最寄駅、リモート可能日数(週1〜5日)、福利厚生、残業時間の目安、求人要件、志望順位・志望理由、決め手・懸念点
- `status_history`: 選考ステータスが変わるたびに追加される履歴(ステータス、実施日時、Googleカレンダー予定ID)。過去の記録は消えず、`companies.status` は常に最新値を反映
- `interview_stages`: 会社に紐づく選考ステージ(ステージ名、選考日程、実施方法、面接官名、会話内容、面接の印象、結果、Googleカレンダー予定ID)
